...
重建影响、全基因可用性、Moran 与 AUCell 的科学比较及产物定义统一见
[reconstruction-impact](../reconstruction-impact/README.md)。本目录只维护通用批量协议。
...
Scientific aspect implementation and acceptance are tracked by the
[reconstruction-impact plan](../reconstruction-impact/README.md). An explicitly
requested aspect without an adapter reports `not_implemented`; this does not
imply completion of the planned scientific analyses.
