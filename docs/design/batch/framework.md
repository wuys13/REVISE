...
分析参数、资源或实现变化只影响相关分析；不触发重建。重建输入或有效配置变化使重建及依赖分析失效。框架按源文件识别实现，不追踪完整的传递调用图；模块通过 `version` 明确标识未被自动覆盖的外部依赖变更。同一 Python 解释器已加载的 adapter 源码若发生变化，任务明确失败并要求重启解释器，再用新源码重跑；不会把缓存旧函数登记为新实现的结果。
...
本框架不执行数据格式转换、标签 harmonization 或其他分支正在开发的科学算法。示例 adapter 和调用回归证明框架可接入，不证明生物学结论有效。
