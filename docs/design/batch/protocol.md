...
### Reconstruction expression view

`inputs.reconstruction_expression(strategy="auto")` 返回与空间观测对应的完整重建基因视图。hST 与 iST mean/random 使用原生 SVC；iST paired 使用 `cluster_mean_projection`：先计算 donor cluster × 全基因均值，再按真实空间单元的 cluster 映射。禁止在 paired 下请求 `native`，也不将 donor 行解释为空间单元。sST 不使用此入口，需按上节角色选择原生载体或下节父 spot 基线。

该视图保留 `X` 原始尺度、完整 donor gene axis、实际空间坐标以及投影/原生载体 provenance，不负责 normalization 或评分。AUCell 等表达分析使用投影后的表达计算；对 donor 评分后再求均值不是同一操作。全基因科学要求由 [reconstruction-impact 输入定义](../reconstruction-impact/input-views.md) 统一维护。
