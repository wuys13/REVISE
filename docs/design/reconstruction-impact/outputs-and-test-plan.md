...
anatomy candidate map, per-window Raw / Recon / delta `Neff`, anatomy-stratified
Region summaries, threshold and scale sensitivity tables.
...
invalid windows, anatomy candidates/context summaries, Region area/fractions,
config carrier-role validation, and notebook content boundaries. Existing
