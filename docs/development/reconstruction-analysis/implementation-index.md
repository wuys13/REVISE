...
| 表达 | 含义 |
|---|---|
| **已落实：工程通过；真实样本待验收** | 生产代码和针对性测试已存在，但仍需要真实输入、科学解释或用户验收。 |
...
| **R6 比较 Notebook 与最终默认值** | **工程完成；Notebook fixture 通过，科学比较后置** | T/Macro/CAF 重建后重新 Leiden，与原空间 subtype 比较；不把旧标签当新结果。 | [Notebook](../../../reproduce/case/assembly_comparison.ipynb)、[计算模块](../../../revise/analysis/assembly_comparison.py)、[测试](../../../tests/analysis/test_assembly_comparison.py)。 | 小 fixture 执行 Notebook、共同轴/缺失、列联表/ARI/NMI/图表；真实运行和默认值另定。 |
...
本批工程结果：**964 项测试 + 24 项子测试通过**，真实样本与科学验收未执行。[验证记录](verification-ot-assembly-2026-09-19.json) · [配置与使用](plans/ot-assembly.md)。
