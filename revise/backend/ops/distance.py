...
def similarity_to_distance(
    similarity: Any, valid_neighbor_mask: Any
) -> np.ndarray:
    """Convert valid graph similarities into minimization costs."""
    ...
    if valid.shape != similarity.shape:
        raise ValueError(
            f"valid_neighbor_mask shape {valid.shape} "
            f"!= similarity shape {similarity.shape}"
        )

    distance = np.full(similarity.shape, np.inf, dtype=np.float64)
    if not np.any(valid):
        return distance

    values = similarity[valid]
    if not np.all(np.isfinite(values)):
        raise ValueError("valid similarities must be finite")
    if np.any(values <= 0):
        raise ValueError("valid similarities must be strictly positive")

    maximum = float(values.max())
    distance[valid] = np.clip(1.0 - values / maximum, 0.0, 1.0)
    return distance
