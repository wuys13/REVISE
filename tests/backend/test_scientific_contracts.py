...
from revise.backend.ops.posterior_conditioning import align_posterior_categories
from revise.backend.ops.posterior_conditioning import get_posterior_matrix
from revise.backend.ops.posterior_conditioning import posterior_affinity
from revise.backend.ops.posterior_conditioning import posterior_conditioning_strict
...
def _load_impute_affinity():
    return _load_functions(
        "revise/backend/runners/sc_svc_impute_benchmark.py",
        ["_posterior_columns", "_label_column_index", "_build_impute_posterior_affinity"],
        {
            "align_posterior_categories": align_posterior_categories,
            "get_posterior_matrix": get_posterior_matrix,
            "posterior_affinity": posterior_affinity,
            "posterior_conditioning_strict": posterior_conditioning_strict,
            "np": np,
            "pd": pd,
        },
    )._build_impute_posterior_affinity
...
def _posterior_adata(values, columns, *, subclusters=None):
    values = np.asarray(values, dtype=np.float64)
    obs = pd.DataFrame(index=[f"obs-{i}" for i in range(values.shape[0])])
    if subclusters is not None:
        obs["subcluster"] = subclusters
    adata = AnnData(X=np.ones((values.shape[0], 1)), obs=obs)
...
def _affinity_target(*, strict):
    return SimpleNamespace(
        config=SimpleNamespace(
            ...
            posterior_conditioning_strict=strict,
        ),
        logger=SimpleNamespace(warning=lambda *args, **kwargs: None),
    )


def test_impute_affinity_aligns_reversed_posterior_categories():
    build_affinity = _load_impute_affinity()
    spot = _posterior_adata([[1.0, 0.0]], ["A", "B"])
    reference = _posterior_adata(
        [[0.0, 1.0]],
        ["B", "A"],
...
@pytest.mark.parametrize(
    ("spot_columns", "reference_columns", "message"),
    [
        (["A", "B"], ["A", "C"], "missing=['B'], extra=['C']"),
        (["A", "B"], ["A"], "missing=['B'], extra=[]"),
        (["A"], ["A", "B"], "missing=[], extra=['B']"),
        (["A", "B"], ["A", None], "null category labels"),
        (["A", "B"], ["A", "A"], "duplicate category labels"),
        (["A", None], ["A", "B"], "null category labels"),
        (["A", "A"], ["A", "B"], "duplicate category labels"),
        (["A", "B"], ["A/B", "A_B"], "collide after '/' normalization"),
        (["A/B", "A_B"], ["A", "B"], "collide after '/' normalization"),
    ],
)
def test_impute_affinity_rejects_invalid_semantic_category_axes(
    spot_columns,
    reference_columns,
    message,
):
    build_affinity = _load_impute_affinity()
    spot = _posterior_adata(
        [np.ones(len(spot_columns))],
        spot_columns,
    )
    reference = _posterior_adata(
        [np.ones(len(reference_columns))],
        reference_columns,
        subclusters=["cluster-1"],
    )
...
def test_impute_affinity_uses_one_hot_fallback_when_only_spot_axis_is_labeled(
    strict,
):
    build_affinity = _load_impute_affinity()
    spot = _posterior_adata([[1.0, 0.0]], ["A", "B"])
    ...
    affinity = build_affinity(
        _affinity_target(strict=strict),
        spot,
        reference,
        "subcluster",
        ["cluster-1"],
...
@pytest.mark.parametrize("strict", [True, False])
def test_impute_affinity_rejects_labeled_reference_with_unlabeled_spot_axis(
    strict,
):
    build_affinity = _load_impute_affinity()
    spot = _posterior_adata([[1.0, 0.0]], ["A", "B"])
    ...
    spot.obsm["Level1"] = spot.obsm["Level1"].to_numpy()

    warnings = []
    target = _affinity_target(strict=strict)
    target.logger.warning = lambda *args: warnings.append(args)

    if strict:
        with pytest.raises(ValueError, match="labeled category axes"):
            build_affinity(
                target,
                spot,
                reference,
...
def test_impute_affinity_preserves_unlabeled_positional_compatibility():
    build_affinity = _load_impute_affinity()
    spot = _posterior_adata([[1.0, 0.0]], ["A", "B"])
    reference = _posterior_adata(
        [[1.0, 0.0]],
...
def test_impute_affinity_preserves_missing_reference_posterior_fallback():
    build_affinity = _load_impute_affinity()
    spot = _posterior_adata([[1.0, 0.0]], ["A", "B"])
    reference = AnnData(
        X=np.ones((1, 1)),
        obs=pd.DataFrame({"subcluster": ["cluster-1"]}, index=["obs-0"]),
    )
...
@pytest.mark.parametrize(
    ("columns", "message"),
    [
        (["A", None], "null category labels"),
        (["A", "A"], "duplicate category labels"),
        (["A/B", "A_B"], "collide after '/' normalization"),
    ],
)
def test_impute_affinity_validates_spot_axis_before_one_hot_fallback(
    columns,
    message,
):
    build_affinity = _load_impute_affinity()
    spot = _posterior_adata([np.ones(len(columns))], columns)
    reference = AnnData(
        X=np.ones((1, 1)),
        obs=pd.DataFrame({"subcluster": ["cluster-1"]}, index=["obs-0"]),
    )
